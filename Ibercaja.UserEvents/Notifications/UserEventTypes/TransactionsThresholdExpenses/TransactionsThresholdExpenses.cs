...
{
    public class TransactionsThresholdExpensesNotification : INotificationProvider
    {
        private static readonly ILog Logger = LogManager.GetLogger(typeof(TransactionsThresholdExpensesNotification));
        ...
        public TransactionsThresholdExpensesNotification(ICoreContextProvider contextProvider, IAccountsManager accountsManager, ITransactionManager transactionManager)
        {
        ...
        public Notification GetNotification(string userIdentifier, long userId, IDictionary<string, object> userEvent, long userEventId, DateTime? createdEvent, string message)
        {
            ...
            var currency = userEvent["CurrencyCode"] as string;
            ...
            var notificationData = new
            {
                ...
                NotificationType = IbercajaUserEventTypes.TransactionsThresholdExpenses
            };
            ...
            //$"¡Hey! Tienes un cargo en tu {accountType} de {amount}€"

            var notification = new Notification
            {
                UserNici = userIdentifier,
                SourceId = $"{userIdentifier}-{userEventId}",
                NotificationType = IbercajaUserEventTypes.TransactionsThresholdExpenses + "." + typeOfBatch,
        ...
        public string GetTypeOfBatch(long userId, long userEventId)
        {
            ...
            using (ICoreUserContext userContext = _contextProvider.UserContext(userId))
            {
                ...
                {
                    Logger.Info($"Expenses: batchType query for userId: {userId} and userEventId: {userEventId}");
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        ...
                        try
                        {
                            while (reader.Read())
                            {
                                ...
                                Logger.Info($"Expenses batchType: {batchType}");
                                switch (batchType)
                                {
                                    case "BA":
                                        batchType = "1";
                                        Logger.Info($"Expenses batchType Final: {batchType}");
                                        return batchType;
                                    case "BT":
                                        batchType = "1";
                                        Logger.Info($"Expenses batchType Final: {batchType}");
