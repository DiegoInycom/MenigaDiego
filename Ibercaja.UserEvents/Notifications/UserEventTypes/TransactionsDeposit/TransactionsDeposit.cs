...
{
    public class TransactionsDepositNotification : INotificationProvider
    {
        private static readonly ILog Logger = LogManager.GetLogger(typeof(TransactionsDepositNotification));
        ...
        public TransactionsDepositNotification(ICoreContextProvider contextProvider, IAccountsManager accountsManager, ITransactionManager transactionManager)
        {
        ...
        public Notification GetNotification(string userIdentifier, long userId, IDictionary<string, object> userEvent, long userEventId, DateTime? createdEvent, string message)
        {
            ...
            // User Event
            // (long)TransactionId, (double)Amount, (string)Description, (string)CurrencyCode
            var transactionId = userEvent["TransactionId"] as long? ?? 0;
            var description = userEvent["Description"] as string;
            ...
            var notificationData = new
            {
                ...
                NotificationType = IbercajaUserEventTypes.TransactionsDeposit + "." + typeOfBatch
            };
            ...
            //$"¡Bien! Tienes un ingreso en tu {accountType} de {amount}€"

            var notification = new Notification
            {
                UserNici = userIdentifier,
                SourceId = $"{userIdentifier}-{userEventId}", //Guid.NewGuid().ToString(),
                NotificationType = IbercajaUserEventTypes.TransactionsDeposit,
        ...
        public string GetTypeOfBatch(long userId, long userEventId)
        {
            ...
            using (ICoreUserContext userContext = _contextProvider.UserContext(userId))
            {
                ...
                {
                    Logger.Info($"Deposit: batchType query for userId: {userId} and userEventId: {userEventId}");
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        ...
                        try
                        {
                            while (reader.Read())
                            {
                                ...
                                Logger.Info($"Deposit batchType: {batchType}");
                                switch (batchType)
                                {
                                    case "BA":
                                        batchType = "1";
                                        Logger.Info($"Deposit batchType Final: {batchType}");
                                        return batchType;
                                    case "BT":
                                        batchType = "1";
                                        Logger.Info($"Deposit batchType Final: {batchType}");
