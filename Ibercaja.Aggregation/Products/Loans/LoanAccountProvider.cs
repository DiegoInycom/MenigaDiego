...
namespace Ibercaja.Aggregation.Products.Loans
{
    public class LoanAccountProvider : IAccountsProvider
    {
        private const string AccountInformationParameterName = "AccountInformation";
        private const string EurobitsLoanAccountType = "P";
        private const string Relationship = "Relationship0";
        private readonly string _userDocument;

        private static readonly ILog Logger = LogManager.GetLogger(typeof(LoanAccountProvider));

        private readonly IAggregationService _aggregationService;

        public LoanAccountProvider(IAggregationService aggregationService, string userDocument)
        {
        ...
        public IEnumerable<BankAccountInfo> GetBankAccountInfos()
        {
            ...
            foreach (var loan in loans.Where(x => x.AccountType == EurobitsLoanAccountType))
            {
                decimal amount;
                decimal limit;
                    ...
                    && decimal.TryParse(loan.InitialBalance.Value, NumberStyles.Currency, CultureInfo.InvariantCulture, out limit))
                {
                    var b = new BankAccountInfo
                    {
                        ...
                        AccountCategoryDetails = IbercajaProducts.Loan,
                        ...
                        AccountParameters = new List<KeyValuePair<string, string>>
                        {
                            new KeyValuePair<string, string>(
                                ...
                                FormatAccountInformation("", loan.Branch, loan.ControlDigits, loan.AccountNumber)),
                            new KeyValuePair<string, string>(
                                Relationship, 
